  ...
  app.get("/api/cms/analytics/:type", async (req, res) => {
    try {
      ...
      if (type === "drug-spending-b" || type === "drug-spending-d") {
        const yearGroups = {};
        rows.forEach(r => { const y = r.Year || "Unknown"; if (!yearGroups[y]) yearGroups[y] = []; yearGroups[y].push(r); });
        result = {
      ...
      if (type === "telehealth") {
        const groups = {};
        rows.forEach(r => { const k = r.Year + "-" + r.Quarter; if (!groups[k]) groups[k] = []; groups[k].push(r); });
        result = {
...
// Insert before "  return httpServer;"
const marker = "  return httpServer;";
...
const newContent = before + analyticsRoute + "\n" + after;

fs.writeFileSync(filePath, newContent, "utf8");
console.log("Successfully inserted analytics route. File size:", newContent.length, "bytes");
