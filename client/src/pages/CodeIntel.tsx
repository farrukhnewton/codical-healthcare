...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px", color: "var(--text-muted, #6B7280)", display: "flex" }}>
...
function StatCard({ label, value, sub, color = "#15803D" }: any) {
  return (
    <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "16px", padding: "16px", border: "1px solid rgba(255,255,255,0.7)", flex: 1 }}>
      <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted, #6B7280)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "6px" }}>{label}</div>
      <div style={{ fontSize: "22px", fontWeight: 900, color }}>{value}</div>
      {sub && <div style={{ fontSize: "11px", color: "var(--text-muted, #6B7280)", marginTop: "2px" }}>{sub}</div>}
    </div>
  );
}
...
export function CodeIntel() {
  ...
  if (loading) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "400px" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: "40px", height: "40px", border: "3px solid #E2E8F0", borderTop: "3px solid #15803D", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
        <div style={{ fontSize: "14px", color: "var(--text-secondary, #4B5563)", fontWeight: 500 }}>Loading intelligence for {code}...</div>
      </div>
  ...
  if (error || !codeInfo) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "400px" }}>
      <div style={{ textAlign: "center", maxWidth: "320px" }}>
        ...
        <div style={{ fontSize: "16px", fontWeight: 700, color: "var(--text-primary, #111827)", marginBottom: "8px" }}>Code Not Found</div>
        <div style={{ fontSize: "13px", color: "var(--text-secondary, #4B5563)" }}>No data found for code <strong>{code}</strong>. Try searching for a valid ICD-10, CPT, or HCPCS code.</div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "rgba(255,255,255,0.4)", minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderBottom: "1px solid #F1F5F9", padding: "0 24px" }}>
        {/* Breadcrumb */}
        <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "12px 0 0", fontSize: "12px", color: "var(--text-muted, #6B7280)" }}>
          <button onClick={() => setLocation("/search")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", color: "var(--text-muted, #6B7280)", padding: 0, fontSize: "12px" }}>
            <ArrowLeft size={12} /> Search
          </button>
          <ChevronRight size={12} />
          <span style={{ color: "var(--text-primary, #111827)", fontWeight: 600 }}>{code}</span>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "16px 0 0", flexWrap: "wrap", gap: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
            ...
            <div style={{ fontSize: "16px", fontWeight: 600, color: "var(--text-primary, #111827)", maxWidth: "500px", lineHeight: 1.3 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <button onClick={() => setFavorited(f => !f)} style={{ width: "36px", height: "36px", borderRadius: "10px", background: favorited ? "#FEF9C3" : "#F8FAFC", border: `1px solid ${favorited ? "#FDE047" : "#E2E8F0"}`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "12px 0", flexWrap: "wrap" }}>
          ...
          {codeInfo.version && <span style={{ padding: "3px 10px", background: "#F1F5F9", color: "var(--text-secondary, #4B5563)", borderRadius: "20px", fontSize: "11px", fontWeight: 600 }}>{codeInfo.version?.toUpperCase()}</span>}
        ...
        {/* Tabs */}
        <div style={{ display: "flex", gap: "0", borderTop: "1px solid #F1F5F9", marginTop: "4px" }}>
          {TABS.map(tab => {
      ...
      <div style={{ padding: "24px", maxWidth: "1000px" }}>
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.15 }}>
            ...
            {activeTab === "overview" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "20px", border: "1px solid rgba(255,255,255,0.7)" }}>
                  ...
                  <div style={{ fontSize: "15px", color: "var(--text-primary, #111827)", lineHeight: 1.6, fontWeight: 400 }}>
                    {String(codeInfo.description || "").replace(/^"|"$/g, "")}
                  </div>
                  {codeInfo.procedureDetails && (
                    <div style={{ marginTop: "12px", padding: "12px", background: "rgba(255,255,255,0.4)", borderRadius: "10px", fontSize: "13px", color: "var(--text-secondary, #4B5563)", lineHeight: 1.6 }}>
                ...
                {data?.relatedCodes?.length > 0 && (
                  <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "20px", border: "1px solid rgba(255,255,255,0.7)" }}>
                    <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted, #6B7280)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "12px" }}>Related Codes</div>
                      ...
                      {data.relatedCodes.map((rc: any) => {
                        ...
                        return (
                          <button key={rc.code} onClick={() => setLocation("/intel/" + rc.code)}
                            style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", background: "rgba(255,255,255,0.4)", border: "1px solid rgba(255,255,255,0.7)", borderRadius: "10px", cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
                          ...
                          >
                            ...
                            <span style={{ fontSize: "13px", color: "var(--text-secondary, #4B5563)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{String(rc.description || "").replace(/^"|"$/g, "")}</span>
            ...
            {activeTab === "rvu" && rvu && (
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "24px", border: "1px solid rgba(255,255,255,0.7)" }}>
                  <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted, #6B7280)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "6px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                    {[
                      ...
                      { label: "Total Non-Fac RVU", value: rvu.totalNonFacilityRvu, color: "var(--text-primary, #111827)" },
                      { label: "Total Facility RVU", value: rvu.totalFacilityRvu, color: "var(--text-primary, #111827)" },
                    ].map((item, i) => (
                      <div key={i} style={{ padding: "14px", background: "rgba(255,255,255,0.4)", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.7)" }}>
                        <div style={{ fontSize: "11px", fontWeight: 600, color: "var(--text-muted, #6B7280)", marginBottom: "4px" }}>{item.label}</div>
                  ...
                  <div style={{ marginTop: "12px", padding: "12px 16px", background: "rgba(255,255,255,0.4)", borderRadius: "10px", fontSize: "12px", color: "var(--text-secondary, #4B5563)", display: "flex", gap: "16px", flexWrap: "wrap" }}>
            ...
            {activeTab === "ncci" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "24px", border: "1px solid rgba(255,255,255,0.7)" }}>
                  <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted, #6B7280)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "6px" }}>
                    <Shield size={11} /> NCCI Edit Check
                  </div>
                  <div style={{ fontSize: "13px", color: "var(--text-secondary, #4B5563)", marginBottom: "16px" }}>
                    Enter a second CPT code to check if NCCI edits exist between <strong>{code}</strong> and another code.
                  </div>
                  <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                    <div style={{ padding: "10px 16px", background: "#F1F5F9", borderRadius: "10px", fontSize: "15px", fontWeight: 700, fontFamily: "monospace", color: "var(--text-primary, #111827)" }}>{code}</div>
                    ...
                    <input
                      ...
                      style={{ height: "42px", padding: "0 14px", border: "2px solid #E2E8F0", borderRadius: "10px", fontSize: "15px", fontFamily: "monospace", fontWeight: 600, outline: "none", width: "180px" }}
                      onFocus={e => { e.target.style.borderColor = "#15803D"; }}
                      onBlur={e => { e.target.style.borderColor = "#E2E8F0"; }}
                    />
                    <button onClick={checkNcci} disabled={ncciLoading || !ncciCode2.trim()}
                      style={{ height: "42px", padding: "0 20px", background: ncciCode2.trim() ? "#15803D" : "#F1F5F9", color: ncciCode2.trim() ? "white" : "#94A3B8", border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: 700, cursor: ncciCode2.trim() ? "pointer" : "default", display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ marginTop: "20px" }}>
                      {ncciResult.hasEdit ? (
                        <div style={{ padding: "16px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "12px" }}>
                          <div style={{ fontSize: "14px", fontWeight: 700, color: "#DC2626", marginBottom: "8px" }}>⚠ NCCI Edit Found</div>
            ...
            {activeTab === "coverage" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {data?.lcds?.length > 0 ? data.lcds.map((lcd: any, i: number) => (
                  <div key={i} style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "16px", padding: "18px", border: "1px solid rgba(255,255,255,0.7)" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                          ...
                          <span style={{ fontSize: "11px", color: "var(--text-muted, #6B7280)", fontFamily: "monospace" }}>{lcd.document_id}</span>
                        </div>
                        <div style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-primary, #111827)", lineHeight: 1.4 }}>{lcd.title}</div>
                        <div style={{ fontSize: "12px", color: "var(--text-secondary, #4B5563)", marginTop: "4px" }}>{String(lcd.contractor_name_type || "").split("\r")[0]}</div>
                      </div>
                      <button onClick={() => setLocation("/coverage")} style={{ padding: "6px 12px", background: "rgba(255,255,255,0.4)", border: "1px solid rgba(255,255,255,0.7)", borderRadius: "8px", fontSize: "12px", color: "var(--text-secondary, #4B5563)", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", flexShrink: 0, fontWeight: 600 }}>
                        <ExternalLink size={12} /> View
                      </button>
                    </div>
                  </div>
                )) : (
                  <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "16px", padding: "32px", border: "1px solid rgba(255,255,255,0.7)", textAlign: "center" }}>
                    <FileText size={32} color="#CBD5E1" style={{ margin: "0 auto 12px" }} />
                    <div style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-primary, #111827)" }}>No LCD Policies Found</div>
                    <div style={{ fontSize: "13px", color: "var(--text-muted, #6B7280)", marginTop: "4px" }}>No local coverage determinations found for {code}</div>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "modifiers" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                {data?.commonModifiers?.length > 0 && (
                  <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "24px", border: "1px solid rgba(255,255,255,0.7)" }}>
                    <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted, #6B7280)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "14px", display: "flex", alignItems: "center", gap: "6px" }}>
                      <Tag size={11} /> Commonly Used Modifiers
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      {data.commonModifiers.map((mod: any) => (
                        <div key={mod.code} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", background: "rgba(255,255,255,0.4)", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.7)" }}>
                          ...
                          <div style={{ fontSize: "12px", color: "var(--text-secondary, #4B5563)", lineHeight: 1.3 }}>{mod.desc}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(12px)", borderRadius: "20px", padding: "24px", border: "1px solid rgba(255,255,255,0.7)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      {data.commonPOS.map((pos: any) => (
                        <div key={pos.code} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", background: "rgba(255,255,255,0.4)", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.7)" }}>
                          ...
                          <div style={{ fontSize: "12px", color: "var(--text-secondary, #4B5563)" }}>{pos.desc}</div>
                        </div>
                      ))}
                    </div>
                  </div>
